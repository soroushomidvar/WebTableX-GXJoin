...
def get_matching_rows_for_table(tables, cols, q_start=3, q_end=16, swap_src_target=False, fast_mode=False, force_swap=0):
    ...
    tmp, is_swapped = get_matching_rows(src, target, q_start, q_end, swap_src_target, force_swap)
...
def get_matching_rows(src, target, q_start=3, q_end=16, swap_src_target=False, force_swap=0):
    is_swapped = False
    if (force_swap != -1 and swap_src_target) or force_swap == 1:
        ...
        if avg_target > avg_src or force_swap == 1:
...
def get_matching_rows_golden(tables, cols, swap_src_target=False, force_swap=0):
    assert cols['src_table'].startswith('src') or cols['src_table'].startswith('target')
    assert cols['target_table'].startswith('src') or cols['target_table'].startswith('target')
