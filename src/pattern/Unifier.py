...
def get_covering_set(inputs, pat_inp, patterns, verbose=True, prioritize=False):
    if verbose:
    ...
    max_tries = 50
    while covered != elements and max_tries > 0:
        max_len = -1
        max_idx = None
        for idx, s in enumerate(pat_inp):
            if len(s - covered) > max_len:
                ...
                max_idx = idx
            elif prioritize and len(s - covered) == max_len:
                tr = patterns[idx]
                if len(tr) < len(patterns[max_idx]):
                    # max_len = len(s - covered) # no need
                    max_idx = idx
                elif len(tr) == len(patterns[max_idx]):
                    current_max = patterns[max_idx]
                    if tr.get_complexity() < current_max.get_complexity():
                        # max_len = len(s - covered) # no need
                        max_idx = idx
                    elif tr.get_complexity() == current_max.get_complexity():
                        if tr.get_generalizability_index() > current_max.get_generalizability_index():
                            # max_len = len(s - covered) # no need
                            max_idx = idx
                        elif tr.get_generalizability_index() == current_max.get_generalizability_index():
                            # print(f"Randomness happened {tr} -> {current_max}")
                            if str(tr) < str(current_max):  # to remove any randomness
                                max_idx = idx

        # subset = max(pat_inp, key=lambda s: len(s - covered))

        old_len_covered = len(covered)

        subset = pat_inp[max_idx]
        if max_idx not in cover:
            cover.append(max_idx)
        covered |= subset

        if len(covered) == old_len_covered:
            max_tries -= 1
